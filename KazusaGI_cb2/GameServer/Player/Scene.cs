...
public class Scene
{
	...
	// Persistent GUIDs and runtime entity ids intentionally live in separate
	// namespaces. These maps are destroyed with the Scene and are therefore
	// the authoritative binding for the current scene instance only.
	private readonly Dictionary<ulong, uint> _avatarEntityIds = new();
	private readonly Dictionary<ulong, uint> _weaponEntityIds = new();
	private SceneEntity? _sceneEntity;
	private uint _teamEntityId;
	private uint _mpLevelEntityId;

	// hk4e SceneEntity::level_config_name_vec_. Order is observable because
	// AbilityComp::addAllDynamicConfigAbilities walks this vector directly for
	// avatar/team abilities. Duplicates are intentionally preserved.
	private readonly List<string> _levelConfigNames = new();
	public IReadOnlyList<string> LevelConfigNames => _levelConfigNames;
	...
	public Scene(Session _session, Player _player, bool newManager = false)
	{
		...
		// Scene::init creates SceneEntity and calls addInitLevelConfig with the
		// SceneExcel level_entity_config before player-owned AbilityComps are built.
		if (resourceManager.SceneExcel.TryGetValue(_player.SceneId, out SceneExcelConfig? sceneExcel) &&
			!string.IsNullOrEmpty(sceneExcel.levelEntityConfig))
		{
			AddInitLevelConfig(sceneExcel.levelEntityConfig);
		}
	}

	/// <summary>
	/// Mirror SceneEntity::addInitLevelConfig's level_config_name_vec_ mutation.
	/// The SceneEntity-specific ability_entry_vec_ side is consumed by
	/// SceneAbilityManager and will be migrated with that manager; the ordered
	/// name vector is authoritative for avatar/team dynamic ability discovery.
	/// </summary>
	public bool AddInitLevelConfig(string levelConfigName)
	{
		if (string.IsNullOrEmpty(levelConfigName) ||
			!resourceManager.ConfigLevelEntityMap.ContainsKey(levelConfigName))
		{
			logger.LogWarning($"findLevelEntityConfig failed, level_entity_config={levelConfigName}");
			return false;
		}

		_levelConfigNames.Add(levelConfigName);
		return true;
	}

	/// <summary>
	/// Mirror SceneEntity::findConfigLevelEntityPtrVec: resolve each stored name
	/// in vector order and omit entries that can no longer be resolved.
	/// </summary>
	public IEnumerable<ConfigLevelEntity> GetConfigLevelEntities()
	{
		foreach (string levelConfigName in _levelConfigNames)
		{
			if (resourceManager.ConfigLevelEntityMap.TryGetValue(levelConfigName, out ConfigLevelEntity? config) &&
				config != null)
			{
				yield return config;
			}
	...
	public SceneEntity GetOrCreateSceneEntity()
	{
		// CB2 stores SceneEntity in scene_entity_ptr_, not entity_map_. Its fixed
		// id (0x13800001) is resolved by Scene::findEntity as a special case.
		return _sceneEntity ??= new SceneEntity(session);
	}

	/// <summary>Mirror Scene::findEntity, including the fixed SceneEntity id.</summary>
	public bool TryFindEntity(uint entityId, out Entity entity)
	{
		if (entityId == SceneEntity.FixedEntityId)
		{
			entity = GetOrCreateSceneEntity();
			return true;
		}
	...
	public AvatarEntity GetOrCreateAvatarEntity(PlayerInfos.PlayerAvatar avatar)
	{
		if (_avatarEntityIds.TryGetValue(avatar.Guid, out uint entityId) &&
			EntityManager.TryGet(entityId, out var existing) && existing is AvatarEntity avatarEntity)
			return avatarEntity;

		var created = new AvatarEntity(session, avatar);
		EntityManager.Add(created);
		_avatarEntityIds[avatar.Guid] = created._EntityId;
		avatar.EntityId = created._EntityId;
		return created;
	}

	public bool TryGetAvatarEntity(ulong avatarGuid, out AvatarEntity? avatarEntity)
	{
		avatarEntity = null;
		if (!_avatarEntityIds.TryGetValue(avatarGuid, out uint entityId))
			return false;
		if (!EntityManager.TryGet(entityId, out var existing) || existing is not AvatarEntity found)
		{
			_avatarEntityIds.Remove(avatarGuid);
			return false;
		}
		avatarEntity = found;
		return true;
	...
	/// <summary>
	/// Mirror Scene::delAvatarAndWeaponEntity. Runtime ids/ability objects are
	/// cleared when an avatar leaves this Scene, while persistent GUID/item
	/// state stays on PlayerAvatar/PlayerWeapon. No death packet is implied.
	/// </summary>
	public IReadOnlyList<uint> RemoveAvatarAndWeaponEntity(PlayerInfos.PlayerAvatar avatar)
	{
		var removed = new List<uint>(2);

		if (_avatarEntityIds.Remove(avatar.Guid, out uint avatarEntityId))
		{
			EntityManager.Unregister(avatarEntityId);
			removed.Add(avatarEntityId);
		}
		avatar.EntityId = 0;
		avatar.LastMoveSceneTimeMs = 0;
		avatar.LastMoveReliableSeq = 0;
		avatar.LastMoveParams.Clear();

		if (avatar.EquipGuid != 0 && player.weaponDict.TryGetValue(avatar.EquipGuid, out var equippedWeapon))
		{
			if (_weaponEntityIds.Remove(equippedWeapon.Guid, out uint weaponEntityId))
			{
				EntityManager.Unregister(weaponEntityId);
				removed.Add(weaponEntityId);
			}
			equippedWeapon.EntityId = 0;
		}

		return removed;
	}

	public WeaponEntity GetOrCreateWeaponEntity(PlayerInfos.PlayerWeapon weapon)
	{
		if (_weaponEntityIds.TryGetValue(weapon.Guid, out uint entityId) &&
			EntityManager.TryGet(entityId, out var existing) && existing is WeaponEntity weaponEntity)
			return weaponEntity;

		var created = new WeaponEntity(session, weapon);
		EntityManager.Add(created);
		_weaponEntityIds[weapon.Guid] = created._EntityId;
		weapon.EntityId = created._EntityId;
		return created;
	}

	public bool TryGetWeaponEntity(ulong weaponGuid, out WeaponEntity? weaponEntity)
	{
		weaponEntity = null;
		if (!_weaponEntityIds.TryGetValue(weaponGuid, out uint entityId))
			return false;
		if (!EntityManager.TryGet(entityId, out var existing) || existing is not WeaponEntity found)
		{
			_weaponEntityIds.Remove(weaponGuid);
			return false;
		}
		weaponEntity = found;
		return true;
	...
	public uint GetWeaponEntityId(ulong weaponGuid)
		=> TryGetWeaponEntity(weaponGuid, out var entity) ? entity!._EntityId : 0;

	/// <summary>Scene::addAvatarAndWeaponEntity.</summary>
	public AvatarEntity AddAvatarAndWeaponEntity(PlayerInfos.PlayerAvatar avatar, bool isEnterScene = false)
	{
		var avatarEntity = GetOrCreateAvatarEntity(avatar);
		if (avatar.EquipGuid != 0 && player.weaponDict.TryGetValue(avatar.EquipGuid, out var weapon))
			GetOrCreateWeaponEntity(weapon);
		return avatarEntity;
	}

	/// <summary>
	/// Rebind EquipComp::weapon_gadget_ptr_ after a weapon swap. The old runtime
	/// WeaponGadget disappears from the scene, while the persistent Weapon item
	/// remains in PACK and receives a fresh entity id if now equipped.
	/// </summary>
	public void OnAvatarWeaponChanged(PlayerInfos.PlayerAvatar avatar)
	{
		foreach (var pair in _weaponEntityIds.ToArray())
		{
			if (!player.weaponDict.TryGetValue(pair.Key, out var candidate) ||
				candidate.EquipGuid != avatar.Guid)
				continue;
			// This one is the current weapon and can stay.
			if (candidate.Guid == avatar.EquipGuid)
				continue;
			EntityManager.Unregister(pair.Value);
			candidate.EntityId = 0;
			_weaponEntityIds.Remove(pair.Key);
		}

		// Remove any stale runtime weapon which was previously owned by this
		// avatar but is now unequipped/swapped elsewhere.
		foreach (var pair in _weaponEntityIds.ToArray())
		{
			if (!player.weaponDict.TryGetValue(pair.Key, out var candidate) ||
				candidate.EquipGuid == avatar.Guid)
				continue;
			if (candidate.EntityId == pair.Value && candidate.Guid != avatar.EquipGuid &&
				!player.GetCurrentLineup().Avatars.Any(a => a.EquipGuid == candidate.Guid))
			{
				EntityManager.Unregister(pair.Value);
				candidate.EntityId = 0;
				_weaponEntityIds.Remove(pair.Key);
	...
	public void ClearPlayerRuntimeEntities()
	{
		foreach (var avatar in player.avatarDict.Values)
		{
			avatar.EntityId = 0;
			avatar.LastMoveSceneTimeMs = 0;
			avatar.LastMoveReliableSeq = 0;
		...
		foreach (var weapon in player.weaponDict.Values)
			weapon.EntityId = 0;
		_avatarEntityIds.Clear();
		_weaponEntityIds.Clear();
		_teamEntityId = 0;
		_mpLevelEntityId = 0;
	}

	public TeamEntity GetOrCreateTeamEntity()
	{
		if (_teamEntityId != 0 && EntityManager.TryGet(_teamEntityId, out var existing) && existing is TeamEntity teamEntity)
			return teamEntity;

		var created = new TeamEntity(session);
		EntityManager.Add(created);
		_teamEntityId = created._EntityId;
		return created;
	}

	public MpLevelEntity GetOrCreateMpLevelEntity()
	{
		if (_mpLevelEntityId != 0 && EntityManager.TryGet(_mpLevelEntityId, out var existing) && existing is MpLevelEntity mpEntity)
			return mpEntity;

		var created = new MpLevelEntity(session);
		EntityManager.Add(created);
		_mpLevelEntityId = created._EntityId;
		return created;
	}

	/// <summary>
	/// Reconstruct the player-owned runtime objects for this Scene. Persistent
	/// avatars/items are never moved into the scene; only fresh entity ids are.
	/// </summary>
	public void InitializePlayerRuntimeEntities()
	{
		GetOrCreateSceneEntity();
		GetOrCreateTeamEntity();
		GetOrCreateMpLevelEntity();

		foreach (var avatar in player.GetCurrentLineup().Avatars)
		{
			AddAvatarAndWeaponEntity(avatar, isEnterScene: true);
		}
	}
	...
	public Entity? FindEntityByEntityId(uint entityId)
		=> TryFindEntity(entityId, out var entity) ? entity : null;
